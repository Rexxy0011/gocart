...
const emailShell = (bodyHtml, footer = 'GoCart - buy and sell anything.') => `
<!doctype html>
<html>
<body style="margin:0;padding:0;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif;background:#f8fafc;">
  <div style="max-width:560px;margin:0 auto;padding:32px 24px;">
...
export function welcomeEmailHtml({ name, siteUrl }) {
    return emailShell(`
      <h1 style="margin:0 0 16px;font-size:20px;color:#0f172a;">Welcome to GoCart, ${firstNameOf(name)}</h1>
      <p style="margin:0 0 16px;color:#334155;line-height:1.55;">
        Your account is live. GoCart is an open marketplace - post anything you
...
export function signinAlertHtml({ name, atIso, ip, device, siteUrl }) {
    ...
    return emailShell(`
      ...
      <p style="margin:0 0 16px;color:#334155;line-height:1.55;">
        Hi ${firstNameOf(name)}, your GoCart account was just signed in.
      </p>
...
export function adminNewUserHtml({ name, email, atIso }) {
    ...
    return emailShell(`
      <h1 style="margin:0 0 16px;font-size:20px;color:#0f172a;">New GoCart signup</h1>
      <table style="width:100%;border-collapse:collapse;font-size:14px;">
        ${row('Name', name || '-')}
        ${row('Email', email)}
        ${row('Registered', lagosTime(atIso))}
      </table>
    `, 'GoCart - admin notification.')
}
