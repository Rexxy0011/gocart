...
export const mapProductRow = (row) => {
    if (!row) return null
    // A paid boost is "on" only while its *_until stamp is in the future.
    // The legacy boolean columns (featured/urgent/bulk_sale) are intent
    // markers; the timestamps are source of truth - so an expired boost
    // reads as off here immediately, even before the cron sweeper clears
    // the booleans. Keeps badges + feed placement in sync with real time.
    const boostActive = (until) => Boolean(until) && new Date(until).getTime() > Date.now()
    return {
        id:           row.id,
        name:         row.name,
        ...
        featured:     boostActive(row.featured_until),
        urgent:       boostActive(row.urgent_until),
        bulkSale:     boostActive(row.bulk_sale_until),
...
export const PRODUCT_WITH_STORE_SELECT = `
    ...
    service, vehicle, vin, phone, imei, created_at, updated_at, store_id, bumped_at,
